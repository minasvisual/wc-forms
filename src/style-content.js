export const defaultStyles = `.wc-form-outer {
  ...
  .wc-form-autocomplete-list {
    ...
    margin-top: 4px;
    z-index: 10;
    /* Same rule as .wc-form-select: pin the popup palette so an OS dark preference does
       not bleed into a light page. Canvas and CanvasText must resolve from the SAME
       color-scheme, otherwise the background follows the OS and the text follows the
       page, which is how this list ended up black-on-black. */
    color-scheme: var(--wcf-autocomplete-color-scheme, light);
    ...
    overflow-y: auto;
    color: var(--wcf-autocomplete-color, CanvasText);
  }
